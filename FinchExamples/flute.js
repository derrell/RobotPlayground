...
{
  // Get frequency to play, based on what "buttons" are pressed
  var tone = getFrequency(e.getData());
	
  // Show the frequency being played
  sensors["frequency"].setValue(tone.toString());

  // Play the desired tone. If it is 0 (off), then set duration to 0.
  if (tone != 0)
  {
  ...
  // Save the last tone to avoid re-calling playTone if already stopped
  lastTone = tone;
}
...
this.getRoot().add(grid, { left : 10, top : 10 });
...
grid.add(power, { row : row++, column : 0 });

// Populate the sensor array with form fields for debugging
sensors["obstacle.left"] = new qx.ui.form.TextField();
sensors["obstacle.right"] = new qx.ui.form.TextField();
sensors["light.left"] = new qx.ui.form.TextField();
sensors["light.right"] = new qx.ui.form.TextField();
sensors["frequency"] = new qx.ui.form.TextField();

for (sensorName in sensors)
  {
    grid.add(new qx.ui.basic.Label(sensorName), { row : row, column : 0 });
    grid.add(sensors[sensorName], { row : row++, column : 1 });
  };
